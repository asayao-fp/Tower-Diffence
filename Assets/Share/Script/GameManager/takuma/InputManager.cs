...
=======
    public GameObject[] myArea = new GameObject[5];
>>>>>>> hirata_branch
...
=======
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            { //アイコンをタッチしていた場合
                ...
                for (int i = 0; i < tgls.Length; i++)
                {
                    if (tgls[i].name.Equals(current.name))
                    { //タッチしているアイコンの名前とアイコンが一致した時
                        if (tgls[i].GetComponent<GenerateBarManager>().getGenerate())
                        {
                            tgls[i].isOn = true;
                            select_num = i;
...
=======
        ...
        else if (Input.GetMouseButtonUp(0))
        {
            ...
            if (Physics.Raycast(ray2, out hit))
            {
                ...
                // Debug.DrawRay(ray2.origin, ray2.direction * 1000, Color.red, 3, false);
                // if (hit.collider.gameObject.GetComponent<StageCost>() != null)
                //     hit.collider.gameObject.GetComponent<StageCost>().addCost(1);
            }
            ...
            if (isSelect && canSelect)
            {
                ...
                {
                    if (hit.collider != null && hit.collider.GetComponent<StageCost>() != null)
                    {
                        Debug.Log("addCost " + hit.collider.gameObject.name);
                        hit.collider.gameObject.GetComponent<StageCost>().addCost(1);
                    }
                    //|| isMoving){

                    foreach (GameObject item in myArea)
                    {
                        //item.GetComponent<StageCost>().addCost(1);
                    }
